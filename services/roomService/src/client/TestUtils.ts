...
export function createSocketClient(
  ...
  const playerUpdatedPromise = new Promise<RemoteServerPlayer>(resolve => {
    socket.on('playerUpdated', (player: RemoteServerPlayer) => {
      resolve(player);
    });
  });
  const playerAskedToBecomeAdminPromise = new Promise<RemoteServerPlayer>(resolve => {
    socket.on('playerAskedToBecomeAdmin', (player: RemoteServerPlayer) => {
      resolve(player);
    });
  });
  const playerRemovedPromise = new Promise<void>(resolve => {
  ...
  return {
    ...
    playerUpdate: playerUpdatedPromise,
    playerAskedToBecomeAdmin: playerAskedToBecomeAdminPromise,
    playerRemoved: playerRemovedPromise,
  };
}

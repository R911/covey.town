...
describe('TownServiceApiSocket', () => {
  ...
  it('Informs all players when a player is updated', async () => {
    const town = await createTownForTesting();
    const joinData = await apiClient.joinTown({
      coveyTownID: town.coveyTownID,
      userName: nanoid(),
    ...
    const { socketConnected, playerUpdate } = TestUtils.createSocketClient(
      server,
      joinData.coveySessionToken,
      town.coveyTownID,
    );
    ...
    await Promise.all([playerUpdate, playerUpdate2]);
    expect((await playerUpdate)._id).toBe('321');
    expect((await playerUpdate2)._id).toBe('321');
  });
  ...
  it('Disconnects a player when an admin bans them', async () => {
    const town = await createTownForTesting();
    const joinData = await apiClient.joinTown({
      coveyTownID: town.coveyTownID,
      userName: nanoid(),
